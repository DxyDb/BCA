...
@dataclass
class CirculantConfig(PeftConfig):
    """
    This is the configuration class to store the configuration of a [`CirculantModel`].

    Args:
        block_size (`int`): IFFT frequency dimension.
    ...
    block_size: int = field(default=100, metadata={"help": ""})
    ...
    init_circulant_weights: bool | Literal["gaussian", "loftq"] = field(
        default=True,
        metadata={
            "help": (
    ...
    def __post_init__(self):
        self.peft_type = PeftType.CIRCULANT
        self.target_modules = (
            set(self.target_modules) if isinstance(self.target_modules, list) else self.target_modules
        )
        print(f"[CirculantConfig] [__post_init__]: {self.target_modules}")
